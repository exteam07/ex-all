...
def main(num):
    ...
    url='https://redthryssa.xyz/circle.php?id='+str(num)
    req=requests.get(url,headers=head)
    data=req.json()
    if data:
        name=data['data'][0]['msisdn'] if data['data'] else None       
        gender=data['data'][0]['nickname'] if data['data'] else None                   
      ...
      #  address=data['data'][0]['addresses'][0]['address']  if data['data'][0]['addresses'] else None
       # img=data['data'][0]['image'] if data['data'] else None   
        info = [img,name,phone,carrier,email,address]
       # info = ("name: ", name, )
